...
class AuthService:
    ...
    async def send_sms_code(self, phone_number: str):
        code = str(random.randint(0, 9999)).zfill(4)
        key = f"sms_code:{phone_number}"
        ttl = 300
        await self.redis.set(key, code, ex=ttl)
    ...
    async def resend_sms_code(self, phone_number: str):
        key = f"sms_cooldown:{phone_number}"
        is_on_cooldown = await self.redis.get(key)

        if is_on_cooldown:
            cooldown = await self.redis.ttl(key)
            raise HTTPException(
                status_code=400,
                detail=f"Слишком часто! Подождите {cooldown} секунд перед повторной отправкой",
            )

        await self.send_sms_code(phone_number=phone_number)
    ...
    async def update_user(self, uow: IUnitOfWork, user_id: int, tokens_pair: dict):
        ...
        await uow.users.update(id=user_id, data=updated_user_dict)  # type: ignore
        await uow.commit()

    async def verify_phone_by_code(
        self, uow: IUnitOfWork, phone_number: str, code: str
    ):
        key = f"sms_code:{phone_number}"
        ...
        async with uow:
            user = await uow.users.get_user_by_phone(phone_number=phone_number)  # type: ignore
            if not user:
                new_user = UserSchemaAdd(name="Guest", phone_number=phone_number)
                new_user_dict = new_user.model_dump()
                user = await uow.users.add_one(new_user_dict)  # type: ignore

            tokens_pair = self.create_tokens_pair(user_id=user.id)

            await self.update_user(uow=uow, user_id=user.id, tokens_pair=tokens_pair)

            return tokens_pair

    async def refresh_token_pair(self, uow: IUnitOfWork, refresh_token: str):
        async with uow:
            user = await uow.users.get_user_by_refresh_token(
                refresh_token=refresh_token
            )
    ...
    async def logout(self, uow: IUnitOfWork, refresh_token: str):
        async with uow:
            user = await uow.users.get_user_by_refresh_token(
                refresh_token=refresh_token
            )
